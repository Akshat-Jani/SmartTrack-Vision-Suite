...
from streamlit_app.upload import create_run_dir, save_video, run_tracker_on_video
from streamlit_app.analyzer_wrapper import process_run
import streamlit_app.viz as viz
...
with tab_runs:
    ...
    if res:
        ...
        if res.get("metrics_df") is not None:
            st.plotly_chart(viz.plot_fps_time(res["metrics_df"]), use_container_width=True)
        ...
        if res.get("metrics_df") is not None:
            ...
            st.plotly_chart(viz.plot_per_class_stack_from_detections(res["detections_df"]),use_container_width=True)
